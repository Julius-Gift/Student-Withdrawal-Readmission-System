...
<body class="cont">
	...
	<form method="post" action="clear_apps.php" enctype="multipart/form-data">
		<div class="row">
			<h5 class="ind"><b>APPLICATIONS FOR READMISSION</b></h5><br><br>
			<div>
			<?php
				if (empty($count_read)) {
				...
				if (isset($_GET["feedback"])) {
					if ($_GET["feedback"] == "approvereaderror") {
						echo '<h5 class="error">Response not sent</h5>';
					}
					if ($_GET["feedback"] == "approvedread") {
						echo '<h5 class="suc">Application approved</h5>';
					}
					if ($_GET["feedback"] == "declinereaderror") {
						echo '<h5 class="error">Response not sent</h5>';
					}
					if ($_GET["feedback"] == "declinedread") {
			...
			?>
					...
					<tbody>
						<tr>
							<?php while ($record = mysqli_fetch_assoc($rows)): ?>
								...
								<th> <a href="../students/display_read.php?readId=<?php echo $record["appID"];?>" class=" btn-sm btn-success">
								View</a> </th>
								<td><a href="delete.php?readMsg=<?php echo $record["appID"];?>" class=" btn-sm btn-danger">
								Delete</a></td>
				...
				<button type="submit" id="rem" class="btn btn-danger" name="re-admit"><i class="fas fa-trash"></i>Delete All</button>
			</div>
		</div>
	</form>
</body>
