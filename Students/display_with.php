<?php
	...
	if(isset($_GET["withId"])){
		$appId = $_GET["withId"];
		$sql = "SELECT * FROM withdraw WHERE appId = '$appId'";
...
<body class="cont">
	...
	<main>
		<div class="details">
			...
			<div class="msgs">
				<?php
					if (isset($_GET["with_approve"])) {
						if ($_GET["with_approve"] == "approvenotsubmitted") {
							echo '<h6 class="error">Application not approved, try again later</h6>';
						}
						if ($_GET["with_approve"] == "approvesubmitted") {
							echo '<h6 class="suc">Application approved</h6>';
						}
					}
					if (isset($_GET["with_decline"])){
						if ($_GET["with_decline"] == "declinenotsubmitted") {
							echo '<h6 class="error">Error declining application, try again later</h6>';
						}
						if ($_GET["with_decline"] == "declinesubmitted") {
			...
			<div>
				...
				<span class="spa-disp"><?php echo "$document";?><a target="blank" href="<?php echo "$dir";?>"><i class="fas fa-download"></i></a> 
			</div>
			<div>
			...
			<div class="btns"><br>
				<a href="../staff/with_approve.php?docID=<?php echo $rows["appId"];?>" class="btn-sm btn-success" id="button">Approve</a>
				<a href="../staff/with_decline.php?docID=<?php echo $rows["appId"];?>" class="btn-sm btn-danger" id="button">Decline</a>
		</div>
		</div>
</main>
</style>
